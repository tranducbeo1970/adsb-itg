...
public class SocketClientThread extends BaseThread {
	...
	try {
	    ...
	    running = true;
	    logger.info("Start handling AMHS service");
...
//	    this.socket.bind(new InetSocketAddress(this.address, port));
	    ...
	    logger.error("Connection to %s:% fail", this.address, this.port);
	    logger.error(ex);
	    onStatuSupdate(ConnectionEventType.Disconnected, null);
