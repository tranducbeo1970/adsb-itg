...
public class VVPGraphic extends IDrawableObject{
    ...
    VVPGraphic(VVP vvp) {
        ...
        this.name = vvp.getName();
        this.type = vvp.getType();
        this.radius = vvp.getRadius();
        this.levelmin = vvp.getLevelmin();
        this.weight = vvp.getWeight();
        this.enable = vvp.getEnabled();
        this.zIndex = vvp.getzIndex();
        this.color = new RGB(vvp.getColor());
        this.changed = true;
    }
    ...
    protected void calculate() {
         try {
            ...
            switch (this.type) {
                case "POLYGON":
                    this.size = this.vvp.getPoints().size();
                    this.buffer = Buffers.newDirectFloatBuffer(size * 3);
                    for (Point2f point : this.vvp.getPoints()) {
                ...
                case "CIRCLE":
                    this.size = 360;
                    this.buffer = Buffers.newDirectFloatBuffer(size * 3);
                    Point2f center = new Point2f(this.vvp.getPoints().get(0));
