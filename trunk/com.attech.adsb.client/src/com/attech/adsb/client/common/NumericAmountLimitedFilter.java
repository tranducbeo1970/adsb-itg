 ...
 * @author andh
 */
public class NumericAmountLimitedFilter extends DocumentFilter {

    private int limit;
//    private int limitAmout;
    
    public NumericAmountLimitedFilter() {
        limit = 0;
    }
    
    public NumericAmountLimitedFilter(int limit) {
...
//            if (Character.isLowerCase(c)) c = Character.toUpperCase(c);
            ...
            String textAfterReplacement = new StringBuilder(fb.getDocument().getText(0, fb.getDocument().getLength())).replace(i, i + i1, String.valueOf(c)).toString();
            try {
                int value = Integer.parseInt(textAfterReplacement);
                if (value > limit || value < 0) continue;
//                if (value < 0) {
//                    value = 0;
//                } else if (value > 360) {
//                    value = 360;
