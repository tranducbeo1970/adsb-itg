...
public class CartesianCoordinate {
    ...
    public static int extract(byte [] bytes, int index, CartesianCoordinate coordinate) {
        ...
        int x = Byter.getComplementNumber(new byte[]{bytes[index++], bytes[index++]});
        coordinate.setOriginX(x);
        
        int y = Byter.getComplementNumber(new byte[]{bytes[index++], bytes[index++]});
    ...
    public void calculate(int scalingFactor) {
        x = this.originX * Math.pow(2, -7 + scalingFactor);
        y = this.originY * Math.pow(2, -7 + scalingFactor);
    }
}
